...
test.describe("R6 — actions", () => {
  test("6.1 Add to Bag is visible on every card without scrolling", async ({ page }) => {
    await buildDeck(page, "Shirts", 4);
    const viewport = page.viewportSize()!;
    for (const btn of await page.getByRole("button", { name: /Add to Bag|Added to Bag/ }).all()) {
  ...
  test("6.2 Add to Bag increments the bag and shows in-place confirmation", async ({ page }) => {
    ...
    await expect(activeCard.getByRole("button", { name: "Added to Bag" })).toBeVisible();
  ...
  test("6.3 Add to Bag does not exit or advance the deck", async ({ page }) => {
    ...
    await page.locator('[data-card-active="true"]').getByRole("button", { name: "Add to Bag" }).click();
  ...
  test("6.6 Remove from wishlist is present at the bottom of every card", async ({ page }) => {
    await buildDeck(page, "Shirts", 3);
    await expect(page.getByRole("button", { name: "Remove from wishlist" })).toHaveCount(3);
  });
